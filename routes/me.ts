...
router.get('/', authMiddleware(), async (req: Request, res: Response) => {
    ...
    try {
        const { rows }: any = await pool.query('SELECT id, name, email, role, created_at, updated_at FROM users WHERE id = $1', [result.decoded.id]);
...
router.put('/edit', authMiddleware(), async (req: Request, res: Response) => {
    ...
    try {
        const { rows }: any = await pool.query('UPDATE users SET name = $1, email = $2 updated_at = $3 WHERE id = $4 RETURNING id, name, email, created_at, updated_at', [name, email, new Date(), result.decoded.id]);
        if (!rows.length) return res.status(404).json({ error: 'Users not found' });
        res.json({ data: rows[0] });
    } catch (error: any) {
        return res.status(500).json({ error: error.message });
