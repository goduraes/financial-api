...
router.get('/', authMiddleware(), async (req: Request, res: Response) => {
    ...
    try {
        const { rows }: any = await pool.query('SELECT * FROM transactions WHERE user_id = $1', [token.decoded.id]);
        res.json({ data: rows });
    } catch (error: any) {
...
router.post('/add', authMiddleware(), async (req: Request, res: Response) => {
    ...
    const tag: any = await pool.query('SELECT * FROM tags WHERE id = $1', [tag_id]);
    ...
    try {
        ...
        res.json({ data: { ...rows[0], tag_name: tag.rows[0].name } });
    } catch (error: any) {
        return res.status(500).json({ error: error.message });
    }
});
